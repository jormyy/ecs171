...
class CNN(nn.Module):
    def __init__(self, input_channels, num_classes, conv_filters, kernel_size, 
                 dropout_conv, fc_size, dropout_fc):
        super(CNN, self).__init__()
        
        # Calculate feature map size after convolutions
        map_size = 28  # Starting image size
        for _ in range(3):  # 3 conv layers
            map_size = (map_size + 2 - kernel_size) // 2 + 1  # After conv and pooling
        
        self.features = nn.Sequential(
            # First conv block
            nn.Conv2d(input_channels, conv_filters[0], kernel_size=kernel_size, padding=1),
            nn.ReLU(),
        ...
        self.classifier = nn.Sequential(
            nn.Linear(conv_filters[2] * map_size * map_size, fc_size),
            nn.ReLU(),
...
# Convert results to DataFrame and save
    ...
    final_model = CNN(
        ...
        dropout_conv=best_config['dropout_conv'],
        fc_size=best_config['fc_size'],
